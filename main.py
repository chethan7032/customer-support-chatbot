...
def userchatboot():
    try:
        ...
        if request.method == 'POST':
            ...
            message = f'You: {usertext}\n'  # User's message
            answer = ""
            close_match = get_close_matches(usertext, brain.keys())
            flag = False
            
            if close_match:
                reply = f'Chat Bot: {brain[close_match[0]][0]}\n'  # Bot's reply
                answer = brain[close_match[0]][0]
                flag = True
            else:
                reply = 'Chat Bot: I can\'t find it in my knowledge base.\n'
                answer = "I can't find it in my knowledge base."
            ...
            answer = message + reply  # Combine the two lines
